...
canvas.forEach(canva => {
    observer.observe(canva); // Observe each video
});
...
// Import the required TensorFlow.js and Pose Detection library
async function play_estimator(video, canva, src) {
...
// Function to handle video playback and pose estimation
function handleIntersection(entries, observer) {
    entries.forEach(entry => {
        const video = entry.target; // The video element being observed
        const canvasId = entry.target.nextElementSibling;
        const src = entry.target.currentSrc;

        // console.log(video); // Log the video elemen
        console.log(src);

        if (entry.isIntersecting) {
            console.log("here")
            if (video != null && canvasId != null) {

                console.log("here")
                play_estimator(video, canvasId, src); // Call the pose estimator function
...
//         const element_id = video.id;
//         let id = document.getElementsByTagName("canvas")[0].id;
